  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">{t("en", "me.title")}</h1>
        <form action={signOut}>
          <button
            type="submit"
      ...
      <p className="mt-2 text-sm" style={{ color: "var(--text-secondary)" }}>
        @{profile?.handle || "anonymous"} &middot; {t("en", "me.note")}
      </p>

      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        {[
          { label: t("en", "me.points"), value: String(profile?.points ?? 0), icon: "🏆" },
          { label: t("en", "me.codes"), value: String(referrals.length), icon: "🔗" },
          { label: "Total Votes", value: String(totalVotes), icon: "📊" },
        ].map((stat) => (
          <div
            key={stat.label}
      ...
      <div className="mt-8 space-y-3">
        <h2 className="text-lg font-semibold">My Referral Codes</h2>
        {referrals.length > 0 ? (
          referrals.map((r) => (
            <div
              key={r.id}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 text-xs" style={{ color: "var(--text-muted)" }}>
                  {r.is_verified && <span style={{ color: "var(--success)" }}>Verified</span>}
                  <span>Votes: {r.vote_count}</span>
                </div>
              </div>
            </div>
          ))
          ...
          >
            <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
              You haven&apos;t submitted any referral codes yet.
            </p>
            <a
              href="/en/submit"
