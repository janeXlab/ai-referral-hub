...
export const metadata = {
  title: "AI Referral Hub - Discover & Share AI Referral Codes",
  description:
    "Community-verified invite links, bonus credits, and exclusive deals for 50+ AI products.",
  alternates: {
    ...
    <div className="mx-auto w-full max-w-6xl px-4">
      ...
      <section className="relative py-20 text-center">
        ...
        <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">
          <span className="gradient-text">{t("en", "home.hero.title")}</span>
        </h1>
        <p className="mx-auto mt-6 max-w-2xl text-lg" style={{ color: "var(--text-secondary)" }}>
          {t("en", "home.hero.subtitle")}
        </p>
        ...
        <div className="mt-12 flex items-center justify-center gap-8 sm:gap-12">
          {[
            { num: `${products.length}+`, label: "AI Products" },
            { num: "50+", label: "Referral Codes" },
      ...
      <div className="grid gap-8 pb-16 lg:grid-cols-[1fr_320px]">
        <div className="space-y-10">
          <section>
            ...
            <div className="space-y-3">
              {trending.map((r) => (
                <ReferralCard key={r.id} referral={r} locale="en" showProduct />
              ))}
              {trending.length === 0 && (
                <div className="rounded-xl p-8 text-center text-sm" style={{ background: "var(--bg-card)", color: "var(--text-muted)" }}>
                  No trending referrals yet. Be the first to submit one!
                </div>
              )}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-3">
              ...
              {latest.length === 0 && (
                <div className="rounded-xl p-8 text-center text-sm" style={{ background: "var(--bg-card)", color: "var(--text-muted)" }}>
                  No referrals yet. Be the first to submit one!
                </div>
              )}
            </div>
          </section>
          ...
          >
            <h2 className="text-lg font-semibold">{t("en", "home.how.title")}</h2>
        ...
        <aside className="space-y-4">
          <AdSlot locale="en" size="large" />

          <div
            className="rounded-xl p-4"
            style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)" }}
          >
            <h3 className="font-semibold text-sm mb-3">{t("en", "home.categories")}</h3>
          ...
          <div
            ...
            style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)" }}
          >
            <h3 className="font-semibold text-sm mb-3">Featured Products</h3>
