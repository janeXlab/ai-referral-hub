...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!product) return {};
  return {
    title: `${product.name} 邀请码/邀请链接`,
    description: `获取 ${product.name} 的社区验证邀请码和独家优惠。${product.tagline_zh}。`,
    alternates: {
      languages: {
        en: `/en/products/${slug}`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = {
    ...
    "@type": "Product",
    name: product.name,
    description: product.tagline_zh,
    url: product.website,
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-10">
      ...
      >
        <div>
          <span style={{ color: "var(--text-muted)" }}>邀请码总数: </span>
          <span className="font-semibold">{list.length}</span>
        </div>
        <div>
          <span style={{ color: "var(--text-muted)" }}>已验证: </span>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <section className="space-y-3">
          {list.length ? (
            list.map((r) => <ReferralCard key={r.id} referral={r} locale="zh" />)
          ) : (
            <div
              className="rounded-xl p-8 text-center text-sm"
              style={{ background: "var(--bg-card)", color: "var(--text-muted)" }}
            >
              {t("zh", "product.nocodes")}
            </div>
          )}
        </section>
        ...
        <aside className="space-y-4">
          ...
          >
            <div className="font-semibold mb-2" style={{ color: "var(--text-primary)" }}>
              {t("zh", "product.rules.title")}
            </div>
            <ul className="list-disc space-y-1 pl-5" style={{ color: "var(--text-secondary)" }}>
              <li>{t("zh", "product.rules.1")}</li>
              <li>{t("zh", "product.rules.2")}</li>
              <li>{t("zh", "product.rules.3")}</li>
            </ul>
          </div>
        </aside>
      </div>
