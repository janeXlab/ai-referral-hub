...
import { signInWithGitHub, signInWithGoogle, signInWithEmail } from "@/lib/actions";
...
export default function Page() {
  ...
  return (
    <div className="mx-auto w-full max-w-md px-4 py-10">
      <h1 className="text-2xl font-bold tracking-tight">{t("zh", "signin.title")}</h1>
      ...
      >
        <form action={async () => { "use server"; await signInWithGitHub("zh"); }}>
          <button
            type="submit"
        ...
        <form action={async () => { "use server"; await signInWithGoogle("zh"); }}>
          <button
            type="submit"
            className="flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-medium transition-all cursor-pointer"
            style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-color)" }}
        ...
        <form action={signInWithEmail}>
          ...
          <div className="grid gap-2">
            <label className="text-sm font-medium">{t("zh", "signin.email")}</label>
            <input
              name="email"
              type="email"
              required
              ...
              style={inputStyle}
              placeholder={t("zh", "signin.email.ph")}
            />
          </div>
